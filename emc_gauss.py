...
#    print 'Qint is', type(Qint), shape(Qint)
  #  Qint = np.interp(pp, p*121, Qp[0])
    Qint = np.interp(wave, wg, Qg[0])
...
#    c_init = 0
    ...
    for j in range(0, lenm):
        Qchi2g = np.interp(wave, wg, Qchig[j])
        Qchi2g = n_init*Qchi2g + c_init
        chi2g.append(chisq(frat, Qchi2g, sd = error_frat))
    
#    r_init = a[np.where(chi2 == np.amin(chi2))]
#    v_init = b[np.where(chi2 == np.amin(chi2))]
    m_init = mu[np.where(chi2g == np.amin(chi2g))]
...
#    param = 'r'
#    quit()
    output = open('pickles/gauss_%s_L%s_%s.pkl'%(red,stype[l-1],timestr), 'wb')
    ...
    for tol, n, c in cropchain[np.random.randint(len(cropchain), size=100)]:
      #  Qchain= np.interp(pp, Qp[0], p, method='linear')
        Qchain = np.interp(wave, wg, Qg[0])
        Qchain = Qchain * n * np.pi * m_init[0]**2 + c
...
#    chainfig.savefig('../../../Mie/Miesults/Plots/emceechain_area_s_L%s_%s_%s.pdf'%(st,des, timestr)) 
    chainfig.savefig('Plots/emceechain_gauss_L%s_%s_%s.pdf'%(stype[l-1],red, timestr)) 
...
#    plt.show()
    plt.savefig('Plots/emcee_gauss_L%s_%s_%s.pdf'%(stype[l-1],red, timestr))
