...
# field red Ls
data = asciitable.read('Files/fieldred/fieldred_final.txt')
...
for l in range(0, len(desi)+len(lgname)):
    if l < len(lgname):
        red = lgname[l]
        stype = lgst[l]
        col = lgcol[l]
...
#        samples = cPickle.load(open('pickles/u11538_050323_L5_like_20151215-202111.pkl','rb'))
            ...
            samples = cPickle.load(open('pickles/%s_L%s_like_%s.pkl'%(red,stype,time), 'rb'))
        elif stype == 2:
        ...
        ll = l-len(lgname)
        red = desi[ll]
        if red == 'None' or red == 'J2224438-015852' or red == 'J213952.22+214839.4' or red == 'J231531.39+061714.2':
            continue
        if red == desi[ll-1]:
            continue
        pname = propname[ll]
        ...
        if red == '2MASSJ21481628+4003593' or red == '2MASSWJ2244316+204343':
            war = datar['col1']
            flur = datar['col2']
            fer = datar['col3']
        else:
...
# Deredden low-g object & calculate chi squared
    ...
    plt.plot(waf, flur_norm, 'r', label='%s, $\chi^2$=%3s, DOF=%s'%(pname,format(float('%.3g'%(chi2_lg)),'n'),len(waf)))
    plt.plot(waf, flur_der_norm, 'g', label='dereddened %s, $\chi^2$=%3s, DOF=%s'%(pname,format(float('%.3g'%(chi2_lg_der)),'n'),dofh))
    plt.xlim(0.8, 2.6)
    ...
    plt.savefig('Paper/Figset8/f8_%s.pdf'%(l+1))
    plt.clf()
