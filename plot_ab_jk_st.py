...
#    samples = cPickle.load(open('../../Documents/ASTRO/codes/kay-repo/Python/%s_%s_area_cropchain.pkl'%(des,stype[l]), 'rb'))
    samples = cPickle.load(open('pickles/%s_%s_area_cropchain.pkl'%(red,stype[l]), 'rb'))
    ...
    if stype[l]==0:
        ...
        a_plus0.append(a_mcmc[1])
        a_minus0.append(a_mcmc[2])
        n_emc0.append(n_mcmc[0])
        n_plus0.append(n_mcmc[1])
        n_minus0.append(n_mcmc[2])
        JK0.append(J[l] - K[l])
    if stype[l]==1:
        a_emc1.append(a_mcmc[0])
        a_plus1.append(a_mcmc[1])
        a_minus1.append(a_mcmc[2])
        n_emc1.append(n_mcmc[0])
        n_plus1.append(n_mcmc[1])
        n_minus1.append(n_mcmc[2])
        JK1.append(J[l] - K[l])
    if stype[l]==2:
        a_emc2.append(a_mcmc[0])
        a_plus2.append(a_mcmc[1])
        a_minus2.append(a_mcmc[2])
        n_emc2.append(n_mcmc[0])
        n_plus2.append(n_mcmc[1])
        n_minus2.append(n_mcmc[2])
        JK2.append(J[l] - K[l])
    if stype[l]==3:
        a_emc3.append(a_mcmc[0])
        a_plus3.append(a_mcmc[1])
        a_minus3.append(a_mcmc[2])
        n_emc3.append(n_mcmc[0])
        n_plus3.append(n_mcmc[1])
        n_minus3.append(n_mcmc[2])
        JK3.append(J[l] - K[l])
    if stype[l]==4:
        a_emc4.append(a_mcmc[0])
        a_plus4.append(a_mcmc[1])
        a_minus4.append(a_mcmc[2])
        n_emc4.append(n_mcmc[0])
        n_plus4.append(n_mcmc[1])
        n_minus4.append(n_mcmc[2])
        JK4.append(J[l] - K[l])
    if stype[l]==5:
        a_emc5.append(a_mcmc[0])
        a_plus5.append(a_mcmc[1])
        a_minus5.append(a_mcmc[2])
        n_emc5.append(n_mcmc[0])
        n_plus5.append(n_mcmc[1])
        n_minus5.append(n_mcmc[2])
        JK5.append(J[l] - K[l])

# Calculate J-K color
#    JK.append(J[l] - K[l]-JKavg)#delta J-K
...
for ll in range (0, len(desi)):
    ...
    samples = cPickle.load(open('pickles/%s_%s_area_cropchain.pkl'%(des,st), 'rb'))
    samples[:, 2] = np.exp(samples[:, 2])
    a_mcmc, b_mcmc, s_mcmc, n_mcmc, c_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0\
)))
...
# for delta J-K
#    colo.append(col-JKav[st])
#    colo.append(col)
#    print col-JKav[st], des, st

plota = plt.subplot(111)
#for k in range (0, 6):
plota.errorbar(colo0, ao_emc0, yerr = (ao_plus0, ao_minus0), fmt = markf[0], alpha=0.5, markersize = 8)
plota.errorbar(JK0, a_emc0, yerr = (a_plus0, a_minus0), fmt = marky[0], alpha = 0.8,  markersize = 8)
plt.show()
plt.clf()
plota = plt.subplot(111)
plota.errorbar(colo1, ao_emc1, yerr = (ao_plus1, ao_minus1), fmt = markf[1], alpha=0.5, markersize = 8)
plota.errorbar(JK1, a_emc1, yerr = (a_plus1, a_minus1), fmt = marky[1], alpha = 0.8, markersize = 8)
plt.show()
plt.clf()
plota = plt.subplot(111)
plota.errorbar(colo2, ao_emc2, yerr = (ao_plus2, ao_minus2), fmt = markf[2], alpha=0.5, markersize = 8)
plota.errorbar(JK2, a_emc2, yerr = (a_plus2, a_minus2), fmt = marky[2], alpha = 0.8, markersize = 8)
plt.show()
plt.clf()
plota = plt.subplot(111)
plota.errorbar(colo3, ao_emc3, yerr = (ao_plus3, ao_minus3), fmt = markf[3], alpha=0.5, markersize = 8)
plota.errorbar(JK3, a_emc3, yerr = (a_plus3, a_minus3), fmt = marky[3], alpha = 0.8, markersize = 8)
plt.show()
plt.clf()
plota = plt.subplot(111)
plota.errorbar(colo4, ao_emc4, yerr = (ao_plus4, ao_minus4), fmt = markf[4], alpha=0.5, markersize = 8)
plota.errorbar(JK4, a_emc4, yerr = (a_plus4, a_minus4), fmt = marky[4], alpha = 0.8, markersize = 8)
plt.show()
plt.clf()
plota = plt.subplot(111)
plota.errorbar(colo5, ao_emc5, yerr = (ao_plus5, ao_minus5), fmt = markf[5], alpha=0.5, markersize = 8)
plota.errorbar(JK5, a_emc5, yerr = (a_plus5, a_minus5), fmt = marky[5], alpha = 0.8, markersize = 8)
plt.show()
print len(colo)
quit()
plota = plt.subplot(111)
